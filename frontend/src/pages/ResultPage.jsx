...
function SimilarCase({ name, similarity, img, severity }) {
    const severityLabel =
        severity === '嚴重' ? '高風險' : severity === '無' ? '健康' : severity === '中度' ? '中風險' : '低風險';
    const severityClass = severity === '嚴重' ? 'high' : severity === '無' ? 'none' : 'mid';
    return (
        <div className='similar-case'>
            ...
            {severity && (
                <span className={`similar-case__severity similar-case__severity--${severityClass}`}>
                    {severityLabel}
                </span>
            )}
...
const ResultPage = () => {
    ...
    const { primary, top3, disease_detail: dd, mode, elapsed_sec } = result;
    const isHealthy   = primary?.severity === '無' || primary?.kaggle_class?.toLowerCase().includes('healthy');
        ...
        <>
            ...
            <div className='result-page mt-md-5'>
                <div className='container d-flex flex-column'>
                    {/* ══ 頂部結果橫幅 ══ */}
                    {/* ══ 頂部結果橫幅 ══ */}
                    <div className={`result-hero ${isHealthy ? 'result-hero--healthy' : 'result-hero--disease'}`}>
                        ...
                        <div className='result-hero__time'>辨識時間：{now} {elapsed_sec && ` · 耗時 ${elapsed_sec} 秒`}</div>
                        {mode === 'DEMO' && <div className='result-hero__demo'>⚠️ DEMO 模式</div>}
                        {isLowConf && (
                            <div className='result-hero__low-conf'>
                                <div className='result-hero__low-conf-title'>⚠️ 置信度偏低，辨識結果僅供參考</div>
                                <div className='result-hero__low-conf-desc'>
                                    此植物可能不在支援範圍內，目前支援：
                                    蘋果、藍莓、櫻桃、玉米、葡萄、柳橙、桃、甜椒、馬鈴薯、覆盆子、大豆、南瓜、草莓、番茄
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className='container result-body'>
                        <div className='result-columns'>
                            ...
                            <div className='result-right'>
                                ...
                                {/* 📋 相似病例參考（來自 top3 第2、3名） */}
                                {top3?.length > 1 && (
                                    <div className='r-section'>
                                        <div className='r-section__title'>
                                            <span className='r-section__icon'>📋</span> 相似病例參考
                                        </div>
                                        <p className='r-section__desc'>
                                            以下為 AI 模型辨識的其他候選病害，可作為鑑別診斷參考
                                        </p>
                                        <div className='similar-grid'>
                                            {top3.slice(1).map(item => (
                                                <SimilarCase
                                                    key={item.kaggle_class}
                                                    name={toZh(item.kaggle_class)}
                                                    similarity={Math.round(item.confidence * 100)}
                                                    img={null}
